...
class Graph:
    ...
    def add_links(self, links):
        ...
        for causes, effects, models in links.items:
            ...
            cs, es = causes.copy(), effects.copy()
            ...
            assertion = len_effects == len(effects)
    ...
    def fit_missing_links(self, X, method='xgboost'):
        ...
        for node in pbar:
            ...
            if method == 'xgboost':
                ...
                line1 = mlines.Line2D([], [], color=c1, label='Test', markersize=10, linewidth=0, marker='x')
                line2 = mlines.Line2D([], [], color=c2, label='Train', markersize=10, linewidth=0, marker='.')
                ...
                prediction_dir = os.path.join(figure_dir, 'prediction')
                intermediate_dir = os.path.join(prediction_dir, 'intermediate')
                os.makedirs(intermediate_dir, exist_ok=True)
                savename = intermediate_dir + '/{}_R2_{}.png'.format(node.name, r2.round(3))

                plt.tight_layout()
                plt.savefig(savename)
                print("R2 score : {}".format(r2.round(3)))
                print("Figure saved as " + savename)

                savename = model_dir + "/{}_XGBmodel_{}.pkl".format(node.name, r2.round(3))
                joblib.dump(m, savename)
    ...
    def to_graphviz(self, rankdir="BT"):
        ...
        # nodes_not_noise = [n for n in self.nodes if not n.is_noise_node]
        # for node1 in topo_sort(nodes_not_noise):
    ...
    def draw(self, rankdir="TB", savename = 'tmp.png', rank = None):
        '''
        requires in ipython notebook environment
        '''
        # viz_graph(self.to_graphviz(rankdir=rankdir))
        G = self.to_graphviz(rankdir=rankdir)
        if rank is not None:
            for nodename in G.nodes():
                G.get_node(nodename).attr['rank'] = rank(nodename)
        save_graph(G, name= savename)
...
class CreditFlow:
    ...
    def run_bruteforce_sampling_set(self, source):
        '''
        run shap flow algorithm to fairly allocate credit
        '''
        # random sample valid timelines
        sources = get_source_nodes(self.graph) if source is None else source
        if self.silent:
    ...
    def run_bruteforce_sampling(self, source = None):
        '''
        ...
        for _ in run_range:
            ...
            order = list(np.random.permutation(sources)) if source is None else [node for node in sources if node.name in source]
            if self.verbose:
                print(f"\n----> using order {order}")
                print("background " +\
                      ", ".join(map(lambda node: f"{node}: {node.last_val[0]}",
    ...
    def run(self, method='bruteforce_sampling', len_bg=1, method_type="distributed", source = None):
        '''
        ...
        if method == 'bruteforce_sampling':
            ...
            if method_type == "path":
                self.run_bruteforce_sampling_set(source)
            elif method_type == "distributed":
                self.run_bruteforce_sampling(source)
    ...
    def credit2dot_pygraphviz(self, edge_credit, format_str, idx=-1,
        ...
        for node1, d in edge_credit.items():
            for node2, val in d.items():
                ...
                for node in [node1, node2]:
                    if node not in G:
                        if node.is_noise_node and self.fold_noise:
                            ...
                            formatted_label = f(node.name)
                            if idx < 0 or \
                               not isinstance(node.target, np.ndarray) or \
                               node.is_noise_node:
                                # G.add_node(node, label=f(node.name), shape=shape)
                                G.add_node(node, label=f'<<FONT POINT-SIZE="16">{formatted_label}</FONT>>', shape=shape)
                            else:
                                if show_fg_val:
                ...
                e.attr["fontcolor"] = f"{color}{alpha}"
    ...
    def credit2dot(self, raw_edge_credit,
        ...
        # Re-position my graph according to process rank
        # Assign rank attribute to each node using your rank function
        if rank is not None:
            for nodename in G.nodes():
                G.get_node(nodename).attr['rank'] = rank(nodename)
        ...
        # Create subgraphs for nodes with the same rank
        for rank_value, nodes in rank_groups.items():
...
class GraphExplainer:
    ...
    def shap_values(self, X, method='bruteforce_sampling',
                    skip_prepare=False, rankdir='TB', source=None, **kwargs):
...
def create_xgboost_f(parents, m, **kwargs):
    ...
    def f_(*args):
        ...
        o = m.predict(pd.DataFrame.from_dict(
...
def group_nodes(graph, nodes, name, verbose=False):
    '''Group nodes into a single node with "name" that determines the value of each
